$(document).ready(function(){
    ...
    if (screenfull.enabled) {
        //Set a full screen toggle on a link
        $('#full-screen').click(function(){
                screenfull.toggle();
        });
        //Detect full screen changes and change the img button with it
        document.addEventListener(screenfull.raw.fullscreenchange, function () {
            changeFullScreenButton(screenfull.isFullscreen);
        });
    } else {
        //Can't set full screen so hide the, now useless, button
        $('#full-screen').hide();
    }

});

//Change the full screen button image
function changeFullScreenButton(full) {
    if(full == true){
        imgSrc = "exit-fullscreen-arrows.png";
    } else {
        imgSrc = "fullscreen-arrows.png";
    }
    console.log('full: ' + full);
    var img = $('#full-screen img');
    var pathArray = img.attr("src").split("/");
    pathArray[pathArray.length-1] = imgSrc;
    img.attr("src",pathArray.join("/"));
}
